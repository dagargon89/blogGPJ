...
export default function TagsCreate() {
    ...
    const handleName = (value: string) => {
        setData('name', value);
        if (!data.slug || data.slug === slugify(data.name)) setData('slug', slugify(value));
    };

    const submit = (e: React.FormEvent) => { e.preventDefault(); post('/admin/tags'); };

    return (
        <AppLayout breadcrumbs={[{ title: 'Etiquetas', href: '/admin/tags' }, { title: 'Nueva', href: '#' }]}>
            <div className="mx-auto max-w-xl p-6">
                <h1 className="mb-6 text-2xl font-semibold">Nueva etiqueta</h1>
                <form onSubmit={submit} className="space-y-5">
                    <div>
                        <Label htmlFor="name">Nombre</Label>
                        <Input id="name" value={data.name} onChange={(e) => handleName(e.target.value)} autoFocus />
